...
st.markdown(f"""
 <style>
 @import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600&display=
 .stApp, .stApp p, .stApp h1, .stApp h2, .stApp h3, .stApp button, .stApp textarea, .stApp font-family: 'Inter', sans-serif !important;
 }}
...
if st.session_state.user_name is None:
 ...
 st.markdown(f"""
 <div class='name-card'>
 <p style='font-size:1.6rem; font-weight:300; color:{TEXT}; margin:0;'>Welcome to< <p style='font-size:1.8rem; font-weight:700; background: linear-gradient(90deg, # -webkit-background-clip: text; -webkit-text-fill-color: transparent; ma VectorWare AI
 </p>
...
with st.sidebar:
 ...
 <p style='font-size:1rem; color:#ffffff; margin:8px 0 0 0; font-weight:500;'>Vect </div>
 """, unsafe_allow_html=True)
 # Theme toggle
 ...
 st.markdown("<p class='history-label'> WHAT I REMEMBER</p>", unsafe_allow_html=True for fact in st.session_state.memory_facts[:5]:
 st.markdown(f"<span class='memory-pill'>• {fact[:40]}</span>", unsafe_allow_html= st.markdown("<br>", unsafe_allow_html=True)
 # Grouped chat history
 today, yesterday, this_week, older = group_histories_by_date(st.session_state.chat_histor def render_history_section(label, chats, offset=0):
 if chats:
 st.markdown(f"<p class='history-label'>{label}</p>", unsafe_allow_html=True)
 for i, chat in enumerate(chats[:5]):
 if st.button(f"↩ {chat['title']}", key=f"hist_{label}_{offset}_{i}", use_cont st.session_state.messages = chat["messages"]
 st.rerun()
 if not st.session_state.chat_histories:
 st.markdown("<p style='color:#4b5563; font-size:0.8rem;'>No conversations yet</p>", u else:
 ...
 <p style='color:#00d4ff; font-size:0.7rem; font-weight:600; margin:0 0 2px 0;'> <p style='color:#e5e7eb; font-size:0.75rem; margin:0;'>Llama 4 Scout 17B</p>
 <p style='color:#6b7280; font-size:0.65rem; margin:2px 0 0 0;'>Vision · Search ·  </div>
 """, unsafe_allow_html=True)
 st.markdown("""
 <div class='about-section'>
 <p style='color:#9b59ff; font-size:0.82rem; font-weight:600; margin:0 0 6px 0;'> <p style='color:#9ca3af; font-size:0.75rem; line-height:1.5; margin:0 0 10px 0;'>
 Next-generation AI architecture delivering high-throughput autonomous enterprise  </p>
 <p style='color:#6b7280; font-size:0.7rem; font-weight:600; margin:0 0 4px 0;'>FO <span class='founder-tag'> Samuel Frimpong</span>
 ...
 <p style='color:#4b5563; font-size:0.65rem; text-align:center; margin:12px 0 0 0; </div>
 """, unsafe_allow_html=True)
# ============================================================
# WELCOME DASHBOARD
# ============================================================
if not st.session_state.messages:
 ...
 <p style='color:#9ca3af; font-size:0.95rem; margin-top:0;'>How can I help you tod </div>
 """, unsafe_allow_html=True)
 suggestions = [
 (" ", "Help me design a website for my business"),
 (" ", "Explain forex trading strategies"),
 (" ", "What can AI automation do for my company"),
 (" ", "Write an executive pitch proposal for me"),
 ]
...
for i, msg in enumerate(st.session_state.messages):
 ...
 st.image(base64.b64decode(block["image_url"]["url"].split(",")[1]), w else:
...
st.markdown("<span class='upload-label'> Attach an image for VectorWare to analyze</span>",uploaded_file = st.file_uploader(
 label="upload",
...
if uploaded_file is not None:
 ...
 st.session_state.pending_image = {"base64": b64, "mime_type": mime, "name": uploaded_file st.markdown(f"<div class='img-preview-pill'> {uploaded_file.name} attached</div>", unsa# ============================================================
...
if prompt:
 ...
 {"type": "image_url", "image_url": {"url": f"data:{img['mime_type']};base64,{img[ ]
 with st.chat_message("user"):
 st.write(prompt)
 ...
 if st.session_state.pending_image:
 thinking_steps.append(" Analyzing attached image")
 thinking_steps.append(" Generating response...")
 thinking_html = "<div class='thinking-box'>" + " &nbsp;|&nbsp; ".join(thinking_steps) + " thinking_placeholder.markdown(thinking_html, unsafe_allow_html=True)
 if search_used:
 st.markdown("<span class='search-pill'> Answer includes live web data</span>", unsa # Thinking loader
 ...
 <p style='color:#9b59ff; margin:0; font-size:0.9rem; font-weight:500;'>VectorWare </div>
 """, unsafe_allow_html=True)
 # Memory context
 memory_context = ""
 if st.session_state.memory_facts:
 memory_context = f"\n\nWhat you know about {st.session_state.user_name}: {', '.join(s # System prompt
 system_prompt = f"""You are VectorWare AI, a highly intelligent, professional and friendlThe user's name is {st.session_state.user_name}. Address them by name naturally where it feelBe concise, insightful, and professional. You can analyze images, search the web, and remembe api_messages = [{"role": "system", "content": system_prompt}]
